...
class IST(nn.Module):
    """
    Importance Space formed with Transformer!! (IST)
    """
    ...
    def __init__(self, heads, d_model, num_sumtokens, layers, dropout, max_len, device):
        ...
        self.importance_tokens = nn.Parameter(
        ...
        self.ca = MultiAttentionNetwork(
            self.d_model, self.d_model, self.heads, dropout)
    ...
    def forward(self, x):
        ...
        # cross-attention
        out = self.ca(mem, self.importance_tokens, None)
        # summ = self.deconv(out)
        num_patches = out.size(1)
        summary = self.upsample(out, num_patches, n, bs)
