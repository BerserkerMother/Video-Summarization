...
class NewMAE(nn.Module):
    def __init__(self, heads, d_enc, d_dec, enc_layers, dec_layers, max_len, device):
        ...
        self.mask_token = nn.Parameter(torch.zeros(1, self.d_enc))

        self.first_layer = nn.Linear(self.in_features, self.d_enc)

        self.encoder = Encoder(heads, d_enc, enc_layers)

        self.final_layer = nn.Linear(self.d_enc, self.in_features)

    def new_model_criterian(self, pred, target, mode):
        if mode == 'Sim':
            pass
        else:
    ...
    def forward(self, x, mask_idx, vis_idx):
        ...
        x_mask = self.mask_token + \
            torch.index_select(self.enc_pos_encoding(), 0, mask_idx)
        ...
        reconst = self.final_layer(self.encoder(full_x))
