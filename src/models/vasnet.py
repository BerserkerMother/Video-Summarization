...
class SelfAttention(nn.Module):
    ...
    def __init__(self, apperture=-1, ignore_itself=False, input_size=1024, output_size=1024):
        ...
        self.K = nn.Linear(in_features=self.m,
                           out_features=self.output_size, bias=False)
        self.Q = nn.Linear(in_features=self.m,
                           out_features=self.output_size, bias=False)
        self.V = nn.Linear(in_features=self.m,
                           out_features=self.output_size, bias=False)
        self.output_linear = nn.Linear(
            in_features=self.output_size, out_features=self.m, bias=False)
    ...
    def forward(self, x):
        ...
        if self.apperture > 0:
            ...
            trimask = torch.tril(onesmask, -self.apperture) + \
                torch.triu(onesmask, self.apperture)
...
class VASNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.kb = nn.Linear(
            in_features=self.ka.out_features, out_features=1024)
        self.kc = nn.Linear(
            in_features=self.kb.out_features, out_features=1024)
